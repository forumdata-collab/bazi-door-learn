...
function gotoLesson(id){{
  ...
  if (!lessonEl) return;
  const levelEl = lessonEl.closest('.level-content');
  if (!levelEl) return;
  const lvId = levelEl.id;
  document.querySelectorAll('.level-content').forEach(x => x.classList.remove('active'));
  document.querySelectorAll('.level-tab').forEach(x => x.classList.remove('active'));
  levelEl.classList.add('active');
  const tab = Array.from(document.querySelectorAll('.level-tab')).find(t => t.getAttribute('onclick') && t.getAttribute('onclick').includes(lvId));
  if (tab) tab.classList.add('active');
  setTimeout(() => lessonEl.scrollIntoView({{behavior:'smooth',block:'start'}}), 50);
}}
document.addEventListener('click', function(e){{
  const a = e.target.closest('a[href^="#"]');
  if (!a) return;
  const id = a.getAttribute('href').substring(1);
  if (id.startsWith('lesson') && document.getElementById(id)) {{
    e.preventDefault();
    gotoLesson(id);
  }}
}});
